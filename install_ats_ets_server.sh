...
if [[ "$escolha" == "1" ]]; then
  JOGO="ats"
  APP_ID="$APP_ID_ATS"
  BASE_DIR="/root/ats_server"
elif [[ "$escolha" == "2" ]]; then
  JOGO="ets"
  APP_ID="$APP_ID_ETS"
  BASE_DIR="/root/ets_server"
else
...
# Create start script 2
print_banner "Creating start script 2..."
cat <<EOF > "/root/start_ets2server.sh"
#!/bin/bash
echo "Starting ETS2 Server..."
# Start the server in a screen session
screen -dmS ets2server /root/ets_server/start_server.sh
EOF

chmod +x "/root/start_ets2server.sh"
...
echo -e "\033[1;32mTo start the server, run2:\033[0m"
echo "/root/start_ets2server.sh"